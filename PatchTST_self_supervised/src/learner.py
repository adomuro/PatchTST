...
#### TRANSFORMER George Zerveas et al.



class Learner_TS(GetAttr):
    ...
    def model_forward(self): 
        ...
        self.pred = self.model(X, self.mask)
        self('after_forward')
        return self.pred
...
class Learner_TS_noPatch(GetAttr):
...
def padding_mask(lengths, max_len=None):
    ...
    """
    batch_size = lengths.numel() # returns the total number of elements in 'length' = batch_size
    max_len = max_len or lengths.max_val()  # trick works because of overloading of 'or' operator for non-boolean types
    # create a boolean variables where first it creates a torch variable from 0 to max_len, then make it bigger by multiplying this variables the amount of batch_size row-wise and finally compared if the values inside are smaller than lengths
